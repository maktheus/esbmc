...
| KB-D02 | P0 | **Verificação da Arduino-PID-Library afirmada sem qualquer artefato.** `verification/famous_pid/` aparece vazio porque é um **submódulo quebrado**: está no índice como gitlink (modo `160000`, commit `524a4268`) mas **não existe `.gitmodules`** no repositório. **`KB-F08` resolveu o submódulo e a biblioteca foi recuperada** no commit fixado — `PID_v1.cpp`/`PID_v1.h` estão disponíveis. Isso muda o remédio: a afirmação **não precisa mais ser removida, precisa ser reexecutada e logada**. Rodar o harness com o mock `verification/Arduino.h` e as flags que o artigo declara (`--floatbv --unwind 11`), commitar o log, e só remover a afirmação se o resultado não se confirmar | `artigo/caps/4resultados.tex:252-256`; `git submodule status` | ✅ | `todo` |
