/*
 * gpt2_4x8_qnn.c — QNNVerifier-style FFN verification harness
 *
 * Generated by ffn_qnn_generator.py
 *
 * Architecture: input[4] → Linear → GELU → Linear → output[4]
 *   d_model = 4,  d_ff = 8,  activation = gelu
 *
 ...
 *
 * Verify:
 *   esbmc gpt2_4x8_qnn.c --overflow-check --no-unwinding-assertions --z3
 *
...
int main(void) {
    ...
    /* P1: output within analytically-derived bound ±35 (Q8.8) */
    __ESBMC_assert(output[0] >= -35LL && output[0] <= 35LL, "P1: output[0] out of bound");
    __ESBMC_assert(output[1] >= -35LL && output[1] <= 35LL, "P1: output[1] out of bound");
    __ESBMC_assert(output[2] >= -35LL && output[2] <= 35LL, "P1: output[2] out of bound");
    __ESBMC_assert(output[3] >= -35LL && output[3] <= 35LL, "P1: output[3] out of bound");
