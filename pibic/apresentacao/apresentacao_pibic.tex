...
\begin{frame}[fragile]{Método QNNVerifier --- Injeção de Intervalo Abstrato}
  ...
  \begin{columns}
    ...
    \end{enumerate}
    \begin{exampleblock}{}
      \centering \textbf{Speedup histórico de 18--27$\times$; não remedido}
    \end{exampleblock}
...
\begin{frame}{Resultados --- Verificação FFN (GPT-2 e LLaMA)}
  ...
  \vspace{0.6em}
  \begin{center}
    \textbf{Método 2} --- QNNVerifier (sobreaproximação intervalar)
    \vspace{0.2em}
    ...
    \begin{tabular}{lllll}
      ...
      \midrule
      \texttt{gpt2\_2x4\_qnn.c}     & 2$\times$4$\times$2  & intervalo GeLU & histórico & \textcolor{okgreen}{0.24\,s} \\
      \texttt{gpt2\_4x8\_qnn.c}     & 4$\times$8$\times$4  & intervalo GeLU & histórico & \textcolor{okgreen}{4.24\,s} \\
      \texttt{llama-7b\_4x8\_qnn.c} & 4$\times$8$\times$4  & intervalo SiLU & histórico & \textcolor{okgreen}{3.46\,s} \\
      \texttt{gpt2\_4x16\_qnn.c}    & 4$\times$16$\times$4 & intervalo GeLU & histórico & \textcolor{okgreen}{20.10\,s} \\
      \bottomrule
    \end{tabular}
  \end{center}
  \vspace{0.2em}
...
\begin{frame}{Contribuições Desenvolvidas Nesta Sessão}
  \begin{enumerate}
    ...
    \item \textbf{Otimizações QNNVerifier (FFN/LLM)}\\
          int32 + dead-code removal + fxp32\_mult + Boolector; speedup de 18--27$\times$ é histórico e não foi remedido
  \end{enumerate}
\end{frame}
...
\begin{frame}{Roadmap e Conclusão}
  \begin{columns}
    ...
    \begin{itemize}
      ...
      \item \textcolor{okgreen}{\checkmark} QNNVerifier com tempos atuais medidos; speedup histórico não remedido
    ...
    \begin{exampleblock}{Impacto}
      \begin{itemize}
        \item As propriedades especificadas foram verificadas nos harnesses indicados
        \item Pipeline reutilizável nas topologias MLP ONNX suportadas
        \item Base para escalar a LLMs maiores (slices)
        \item Speedup histórico de 18--27$\times$ requer reprodução controlada
      \end{itemize}
    \end{exampleblock}
