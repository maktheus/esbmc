...
\begin{tikzpicture}[node distance=7mm and 8mm]
  \node[pibitibox] (artifacts) {Artefatos\\checkpoint, JSONs e\\histórico de treinamento};
  \node[pibitiprocess, right=of artifacts] (numeric) {Preparação numérica\\Float32 versus Q8.8\\10.000 estados};
  \node[pibitiprocess, right=of numeric] (network) {Grafo do controlador\\4--24--24--1\\ReLU + tanh aproximada};
  \node[pibitiprocess, below=of network] (harness) {Harnesses C\\domínio Q8.8 e propriedades\\A, B e C};
  \node[pibitiprocess, left=of harness] (bmc) {ESBMC + Z3\\execução simbólica\\e BMC};
  \node[pibitiresult, left=of bmc] (evidence) {Evidências\\veredictos, contraexemplos,\\logs e hashes};
  \node[pibitiresult, below=of bmc] (synthesis) {Síntese científica\\resultado confirmado,\\refutado ou inconclusivo};
  \draw[pibitiarrow] (artifacts) -- (numeric);
  \draw[pibitiarrow] (numeric) -- (network);
  \draw[pibitiarrow] (network) -- (harness);
  \draw[pibitiarrow] (harness) -- (bmc);
  \draw[pibitiarrow] (bmc) -- (evidence);
  \draw[pibitiarrow] (bmc) -- (synthesis);
\end{tikzpicture}
...
\begin{tikzpicture}[node distance=6mm and 4mm,
  ...
  \draw[pibitiarrow] (source) -- (front);
  \draw[pibitiarrow] (front) -- (bmc);
  \draw[pibitiarrow] (bmc) -- (solver);
  \draw[pibitiarrow] (solver) -- (success);
  \draw[pibitiarrow] (solver) -- (failed);
  \draw[pibitiarrow] (solver) -- (inc);
\end{tikzpicture}
