...
\begin{tikzpicture}[node distance=9mm and 9mm]
  \node[pibitibox, text width=3.5cm] (question) {Pergunta central\\o controlador quantizado\\é verificável e confiável?};
  \node[pibitiresult, below left=of question, text width=3.7cm] (numeric) {Fidelidade numérica\\média: 0,0778 N\\máximo: 7,7589 N\\inversão de sinal no pior estado};
  \node[pibitiresult, below=of question, text width=3.7cm] (formal) {Propriedades formais\\força limitada: provada\\A-direita universal: timeout\\replays A e B: reproduzidos};
  \node[pibitiresult, below right=of question, text width=3.7cm] (structure) {Estrutura da rede\\L1: 24/24 contraexemplos\\L2: timeouts parciais\\saída conjunta: inconclusiva};
  \node[pibitiprocess, below=of formal, text width=5.4cm] (conclusion) {Conclusão equilibrada\\há evidência reprodutível de limites e falhas concretas,\\mas não há prova de segurança global do controlador};
  \draw[pibitiarrow] (question) -- (numeric);
  \draw[pibitiarrow] (question) -- (formal);
  \draw[pibitiarrow] (question) -- (structure);
  \draw[pibitiarrow] (numeric) -- (conclusion);
  \draw[pibitiarrow] (formal) -- (conclusion);
  \draw[pibitiarrow] (structure) -- (conclusion);
\end{tikzpicture}
