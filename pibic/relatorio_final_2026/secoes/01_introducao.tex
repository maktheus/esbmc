...
\section{Revisão bibliográfica e estado da arte}
A revisão será organizada em quatro eixos. O primeiro é o \textit{Bounded Model Checking}, no qual a propriedade é analisada em um número finito de passos e o resultado depende das pré-condições, do horizonte e da teoria utilizada. O segundo é a verificação de redes neurais por SMT, incluindo técnicas voltadas a redes quantizadas e ferramentas como QNNVerifier \cite{gopinath2021verifying,meng2022qnnverifier}. O terceiro eixo reúne controle por aprendizado por reforço, representação numérica e efeitos de precisão finita. O quarto trata da transferência de verificações para componentes de IA generativa, distinguindo propriedades do código produzido, propriedades do modelo neural e propriedades do sistema que integra esses componentes.

Essa revisão não deve ser uma enumeração de ferramentas. Cada referência será relacionada a uma decisão metodológica do presente estudo: escolha da representação, definição do domínio, seleção das propriedades, tratamento de contraexemplos ou avaliação de escalabilidade. Referências específicas de DDPG, CartPole, ESBMC e controle seguro deverão ser acrescentadas ao arquivo bibliográfico antes da versão final.
